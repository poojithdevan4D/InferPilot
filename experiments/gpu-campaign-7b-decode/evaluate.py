...
OUT = ROOT / "runs/gpu-campaign-7b-decode"
TOL = 0.02
...
def analyze():
    ...
    for rate in rates:
        ...
        for width in widths:
            ...
            for seed in seeds:
                d = cells.get(f"g7d-default-qps{rate}-a{seed}")
                c = cells.get(f"g7d-dev-qps{rate}-a{seed}-seq{width}")
                if d and c:
    ...
    for rate in rates:
        ...
        for width in widths:
            ...
            for seed in held_seeds:
                d = cells.get(f"g7d-default-qps{rate}-a{seed}")
                c = cells.get(f"g7d-held-qps{rate}-a{seed}-seq{width}")
                if d and c:
                    verdicts[_pareto(d, c)] += 1
            summ = ", ".join(f"{k}×{n}" for k, n in sorted(verdicts.items()))
            maj = verdicts["candidate_dominates"] > sum(verdicts.values()) / 2 if verdicts else False
